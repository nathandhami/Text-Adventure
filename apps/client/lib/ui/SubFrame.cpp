...
SubFrame::SubFrame() : loginButton( "Log In" ) {
...
//	this->layoutGrid.override_background_color( gridColor );
	...
	this->loginButton.signal_clicked().connect( sigc::mem_fun( *this, &SubFrame::loginButton_click ) );
...
void SubFrame::setupEntries() {
	...
	this->usernameEntry.signal_activate().connect( sigc::mem_fun( *this, &SubFrame::loginButton_click ) );
	
	// Password
	this->passwordEntry.set_alignment( 0.5 );
	this->passwordEntry.set_visibility( false );
	this->passwordEntry.signal_activate().connect( sigc::mem_fun( *this, &SubFrame::loginButton_click ) );
}



void SubFrame::loginButton_click() {
